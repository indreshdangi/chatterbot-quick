// backend/server.js
// Multi-provider chat proxy (Groq default Llama-3.1 + Gemini 2.0), no OpenAI.
// Copy this file to backend/server.js and restart: node server.js
...
// Default Groq model (keep cheap llama-3.1 as default)
const GROQ_MODEL_DEFAULT = (process.env.GROQ_MODEL || "llama-3.1-8b-instant").trim();

const GEMINI_KEY = (process.env.GEMINI_KEY || "").trim();
// Default Gemini model (set to gemini-2.0 here; change to exact model id from your Google console if needed)
...
async function callGemini(modelId, userMessage, opts={}) {
  ...
  // Note: Google sometimes uses different endpoints or requires service-account IAM. This uses API-key endpoint.
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // modelKey expected: "llama_3_1_8b_instant" or "gemini_2_0" (we'll interpret)
    ...
    // system prompt: reply in same script as user
    let systemMsg;
    if(containsDevanagari(message)) {
      systemMsg = { role:"system", content: "You are Indresh 2.0. User wrote in Devanagari Hindi. Reply ONLY in Devanagari (हिन्दी). Keep concise and do not repeat the question." };
    } else {
      systemMsg = { role:"system", content: "You are Indresh 2.0. User used Latin script (Hinglish/English). Reply in same script. Do not repeat the user's question." };
    }
    ...
    try {
      if(modelKey === "gemini_2_0" || modelKey.startsWith("gemini")) {
        usedProvider = "gemini";
        providerResult = await callGemini(GEMINI_MODEL, [ systemMsg.content ? systemMsg.content + "\n\n" + message : message ], { maxOutputTokens: 800, temperature: 0.6 });
      } else {
        // default -> Groq Llama 3.1
        usedProvider = "groq";
    ...
    if(p && p.json){
      ...
      // Gemini sometimes returns 'output' with array
      if(!replyText && j?.output && Array.isArray(j.output) && j.output[0] && j.output[0].content) {
        ...
        else if(j.output[0].content && j.output[0].content[0] && j.output[0].content[0].text) replyText = j.output[0].content[0].text;
      }
    } else if(p && p.text) {
      replyText = p.text;
